...
#init logging file
import logging
logging.basicConfig(filename='headache_DinoV2.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
class ADNI3DTo2DSliceDataset(Dataset):
    def __init__(self, transform=None, num_slices=50):
        ...
        data = data[(data['DX'] == 'NL') | (data['DX'] == 'AD')]   # for AD vs NL
        self.data = data.reset_index(drop=True)
        self.transform = transform
        self.num_slices = num_slices
        self.label_map = {'NL': 0, 'AD': 1}
...
class HeadacheDataset(Dataset):
    def __init__(self, csv_path, split='train', transform=None, num_slices=50):
        ...
        Args:
            ...
            split: 'train', 'test', or 'val' to filter by split
        ...
        # Map class names to binary labels (healthy vs headache)
        self.label_map = {
            'HC': 0,         # Healthy
            'HC_MIXED': 0,   # Healthy
            'PPTH': 1,       # Headache
    ...
    def __getitem__(self, idx):
        ...
        #print(f"Processing patient {patient_id} with class {class_name}")
        # Construct image path based on patient ID (adjust this path as needed)
        
        img_path = f'{self.root_dir}/{patient_id}.nii'
        
        # Get binary label (0 for healthy, 1 for headache)
        ...
        for slice_2d in selected_slices:
            ...
            norm_slice = np.uint8(255 * (slice_2d - np.min(slice_2d)) / 
                                (np.max(slice_2d) - np.min(slice_2d) + 1e-5))
            slice_img = Image.fromarray(norm_slice).convert('RGB')
            
            if self.transform:
                slice_img = self.transform(slice_img)
            slices.append(slice_img)

        # Return stacked slices and label
        return torch.stack(slices), label  # Shape: (num_slices, C, H, W)
...
indices = list(range(len(full_dataset)))
labels = [full_dataset.data.loc[i, 'Class'] for i in indices]
train_idx, temp_idx = train_test_split(indices, test_size=0.2, stratify=labels, random_state=42)
temp_labels = [full_dataset.data.loc[i, 'Class'] for i in temp_idx]
val_idx, test_idx = train_test_split(temp_idx, test_size=0.5, stratify=temp_labels, random_state=42)
...
logging.info(f"Train dataset labels: {train_dataset.dataset.data['Class'].value_counts().to_dict()}")
logging.info(f"Validation dataset labels: {val_dataset.dataset.data['Class'].value_counts().to_dict()}")
logging.info(f"Test dataset labels: {test_dataset.dataset.data['Class'].value_counts().to_dict()}")
...
for epoch in range(num_epochs):
    ...
    if (epoch + 1) % 50 == 0:
        ...
        if test_acc > best_test_acc:
            ...
            torch.save(model.state_dict(), 'Models_new/headache_dinov2_best_test_model.pth')
        ...
        plt.legend()
        plt.savefig('Models_new/headache_adni_dinov2_training_loss_updated.png')
        plt.close()
        ...
        plt.savefig(f'Models_new/headache_adni_dinov2_conf_mat_epoch_{epoch+1}.png')
        plt.close()
