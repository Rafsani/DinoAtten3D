...
#init logging file
import logging
logging.basicConfig(filename='ADNI_CLVL_NLvsAD.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
for epoch in range(num_epochs):
    ...
    if (epoch + 1) % 50 == 0:
        ...
        if test_acc > best_test_acc:
            ...
            torch.save(model.state_dict(), 'Models_new/adni_dinov2_best_test_model.pth')
        ...
        plt.legend()
        plt.savefig('Models_new/ADvsNL_adni_dinov2_training_loss_updated.png')
        plt.close()
        ...
        plt.savefig(f'Models_new/ADvsNL_adni_dinov2_conf_mat_epoch_{epoch+1}.png')
        plt.close()
