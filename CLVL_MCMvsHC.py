...
#init logging file
import logging
logging.basicConfig(filename='headache_mcm_DinoV2.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
random.seed(42)
val_mcm     = random.sample(mcm_indices, 20)
val_healthy = random.sample(healthy_indices, 20)
...
for epoch in range(num_epochs):
    ...
    if (epoch+1) % 10 == 0:
        ...
        report  = classification_report(y_true, y_pred, target_names=['Healthy','MCM'])
        ...
        plt.legend()
        plt.savefig('Models_new/headache_mcm_dinov2_loss_plot.png')
        plt.close()
        # — checkpoint on best validation accuracy —
        if val_acc > best_val_acc:
            best_val_acc = val_acc
            torch.save(model.state_dict(), 'Models_new/headache_mcm_dinov2_best_val_model.pth')
