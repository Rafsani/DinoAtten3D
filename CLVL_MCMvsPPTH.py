...
#init logging file
import logging
logging.basicConfig(filename='ppth_mcm_DinoV2.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
for epoch in range(num_epochs):
    ...
    if (epoch+1) % 10 == 0:
        ...
        report  = classification_report(y_true, y_pred, target_names=['MCM','PPTH'])
        ...
        plt.legend()
        plt.savefig('Models_new/ppth_mcm_dinov2_loss_plot.png')
        plt.close()
        # — checkpoint on best validation accuracy —
        if val_acc > best_val_acc:
            best_val_acc = val_acc
            torch.save(model.state_dict(), 'Models_new/ppth_mcm_dinov2_best_val_model.pth')
