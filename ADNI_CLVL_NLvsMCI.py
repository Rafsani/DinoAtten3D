...
#init logging file
import logging
logging.basicConfig(filename='ADNI_CLVL_NLvsMCI.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
class ADNI3DTo2DSliceDataset(Dataset):
    def __init__(self, transform=None, num_slices=50):
        ...
        data = data[(data['DX'] == 'NL') | (data['DX'] == 'MCI')]   # for AD vs NL
        self.data = data.reset_index(drop=True)
        self.transform = transform
        self.num_slices = num_slices
        self.label_map = {'NL': 0, 'MCI': 1}
...
for epoch in range(num_epochs):
    ...
    if (epoch + 1) % 50 == 0:
        ...
        if test_acc > best_test_acc:
            ...
            torch.save(model.state_dict(), 'Models_new/NLvsMCI_adni_dinov2_best_test_model.pth')
        ...
        plt.legend()
        plt.savefig('Models_new/NLvsMCI__adni_dinov2_training_loss_updated.png')
        plt.close()
        ...
        plt.savefig(f'Models_new/NLvsMCI__adni_dinov2_conf_mat_epoch_{epoch+1}.png')
        plt.close()
