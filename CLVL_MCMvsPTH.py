...
#init logging file
import logging
logging.basicConfig(filename='pth_mcm_DinoV2.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
mcm_indices     = [i for i in all_indices if labels[i] == 'MCM']
pth_indices = [i for i in all_indices if labels[i] == 'PTH']

# 2. Sample exactly 10 per class for validation
random.seed(42)
val_mcm     = random.sample(mcm_indices, 10)
val_healthy = random.sample(pth_indices, 10)
val_idx     = val_mcm + val_healthy
...
logging.info(
    f"Val class counts: "
    f"MCM={sum(full_dataset.data.loc[val_idx, 'Class']=='MCM')}, "
    f"pth={sum(full_dataset.data.loc[val_idx, 'Class']=='PTH')}"
)
...
for epoch in range(num_epochs):
    ...
    if (epoch+1) % 10 == 0:
        ...
        report  = classification_report(y_true, y_pred, target_names=['MCM','PTH'])
        ...
        plt.legend()
        plt.savefig('Models_new/pth_mcm_dinov2_loss_plot.png')
        plt.close()
        # — checkpoint on best validation accuracy —
        if val_acc > best_val_acc:
            best_val_acc = val_acc
            torch.save(model.state_dict(), 'Models_new/pth_mcm_dinov2_best_val_model.pth')
